...
const WatchList = () => {
        ...
        const fetchWatchListImages = async () => {
            ...
            if (watchListIds.length > 0) {
                try {
                    ...
                    const watchListImages = allImages.filter(image => watchListIds.includes(image._id));
    ...
    return (
        <div className="sm:columns-1 md:columns-4 gap-3 m-2">
            ...
            {images.length > 0 ? (
                images.map((i) => (
                    <div key={i._id} className="w-full aspect-auto mb-2 relative group cursor-zoom-in">
                        <img className="rounded-2xl w-full" src={i.imageUrl} alt={i.title} />
                        <div className="invisible absolute bottom-3 right-3 group-hover:visible">
                            <button className="bg-slate-50 rounded-full h-8 w-8 text-center">...</button>
                        </div>
                    </div>
                ))
            ) : (
